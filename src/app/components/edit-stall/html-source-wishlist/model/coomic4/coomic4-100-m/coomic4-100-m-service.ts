...
import { C4100MAuthor, C4100MConfig, C4100MData } from './coomic4-100-m';
import { BaseService } from '../../base-service';

@Injectable({
  providedIn: 'root',
})
export class Coomic4100MService extends BaseService<C4100MAuthor> {
  ...
  override processData(rawData: Record<string, string>[], htmlText: string) {
    ...
    rawData.forEach((rawSeries, rowIdx) => {
      ...
      if (authorName) {
        ...
        const key = this.keyForMapping({ stallId, authorName } as C4100MConfig);
      ...
      const promotionalTitle = rawSeries['商品資訊網頁'];
